    ...
    btnAdd.addEventListener("click", () => {
      ...
      if (equipoActivo) {
        ...
        if (
          ...
          image = equipoActivo.acf.foto_del_equipo[0];
        } else {
...
if (document.querySelector(".mySwiper")) {
    ...
    on: {
      init: function () {
        ...
        if (urlParams.has("activeIndex")) {
          ...
          this.slideTo(currentIndex, 600);
          obtenerProductos(texts[currentIndex]);
        } else {
          obtenerProductos(texts[0]);
        }
        function createLines() {
          const windowWidth = document.querySelector(
            ".list .mySwiper .swiper-slide"
          ).offsetWidth;
          const lineWidth = 1; // Ancho de cada línea
          const spaceBetween = 10; // Espacio entre líneas
          // const numLines = Math.floor(
          //   windowWidth / (lineWidth + spaceBetween)
          // );
          const numLines = 30;
  ...
  swiper.on("slideChangeTransitionEnd", function () {
    if (document.querySelector(".list .swiper .swiper-slide-active")) {
      const activeText = document.querySelector(
        ".list .swiper .swiper-slide-active"
      ).innerText;
      const index = texts.indexOf(activeText.toLowerCase());
      // Update the URL with the new activeIndex value
      const newUrl = updateQueryStringParameter("activeIndex", index);
      history.pushState({}, "", newUrl);
      obtenerProductos(texts[index]);
    }
  });
}
// Ejemplo de uso
      ...
      complete: function (anim) {
        ...
        productos.forEach((producto) => {
          ...
          if (
            ...
            image = producto.acf.foto_del_equipo[0];
            image2 = producto.acf.foto_del_equipo[1];
          } else {
            image = "https://placehold.co/600x400?text=Equipos";
        ...
        document.querySelector("#continue").addEventListener("click", () => {
          obtenerProductos(texts[nextIndex]);
          swiper.slideTo(nextIndex, 600);
        });
        anime({
